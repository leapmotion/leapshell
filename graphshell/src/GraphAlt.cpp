...
const float GraphAlt::EDGE_DISTANCE_SMOOTH = 0.975f;
...
void GraphAlt::Draw(const Matrix4x4& textTransform) const {
...
void GraphAlt::UpdateAttractor(int id, const Vector3& position, float strength) {
...
void GraphAlt::createRandomGraph(int numNodes) {
  m_nodes.clear();
  m_edges.clear();
...
int GraphAlt::retrieveOrAddNode(const std::string& name) {
  for (size_t i=0; i<m_nodes.size(); i++) {
...
void GraphAlt::createCompanyGraph() {
  m_nodes.clear();
  m_edges.clear();

  std::string graph = "Tracking	Apps	Components	Platform	QA	Architecture\n\
Apps	Components	Product\n\
Components	Platform	Product	Architecture\n\
Web	Community	Product	Architecture\n\
Platform	QA	Architecture	Firmware\n\
QA	Community	Architecture	Firmware\n\
Community	Product\n\
Product	Hardware\n\
Architecture	Hardware	Firmware\n\
Hardware	Firmware";
...
void GraphAlt::createMusicGraph() {
  ...
  std::ifstream fileInput("music.txt");
  if (!fileInput) {
    return;
  }
  std::string graph((std::istreambuf_iterator<char>(fileInput)), std::istreambuf_iterator<char>());
  ...
  std::cout << "nodes: " << m_nodes.size() << " | edges " << m_edges.size() << std::endl;
}
...
void GraphAlt::resolveConstraints() {
  for (size_t i=0; i<m_nodes.size(); i++) {
    GraphNode& node = m_nodes[i];
...
void GraphAlt::cleanupAttractors() {
...
void GraphAlt::unmarkAllNodesVisited() {
  for (size_t i=0; i<m_nodes.size(); i++) {
...
void GraphAlt::unmarkAllEdgesVisited() {
  for (size_t i=0; i<m_edges.size(); i++) {
...
float GraphAlt::hopsToHighlight(int hops, int searchDepth) {
  if (hops <= searchDepth) {
...
void GraphAlt::bfs(int startIdx, int maxDepth) {
