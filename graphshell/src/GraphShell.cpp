...
void GraphShell::setup()
{
  ...
  m_params->addParam("Charge", &m_graphAlt.m_charge, "min=-10000 max=10000 step=1");
  ...
  m_params->addParam("Min Edge Distance", &m_graphAlt.m_minEdgeDistance, "min=0 max=1000 step=1");
  m_params->addParam("Extra Edge Distance", &m_graphAlt.m_extraEdgeDistance, "min=0 max=1000 step=1");
  m_params->addParam("Weight Power", &m_graphAlt.m_weightPower, "min=1 max=10 step=0.1");
#if USE_LEAP_IMAGE_API
  ...
  m_graphAlt.Start(GraphAlt::COMPANY);
...
void GraphShell::mouseMove(ci::app::MouseEvent e)
{
  if (e.isLeftDown()) {
    ...
    m_viewOffset += 2.0f * (m_dist * (e.getPos() - m_mousePos)) / Globals::windowHeight;
  }
  m_mousePos = e.getPos();
}
...
void GraphShell::mouseWheel(ci::app::MouseEvent e) {
  //m_dist += -50.0f * e.getWheelIncrement();
...
Matrix4x4 GraphShell::getOculusTransform() {
  ...
  Vector3 position = Vector3::Zero();// toVector3(m_Oculus.m_Position);
...
Matrix4x4 GraphShell::getOculusRotation() {
  ...
  Vector3 up = Vector3::UnitY(); // toVector3(m_Oculus.m_FinalUp);
  const Vector3 forward = -Vector3::UnitZ(); // toVector3(m_Oculus.m_FinalForward);
...
void GraphShell::draw() {
  ...
  for (int i=0; i<numEyes; i++) {
    ...
    const ci::Area area = origViewport;
    ...
    if (!m_useOculusRender) {
      ...
      cam.setEyePoint(ci::Vec3f(0, 0, 1000.0f));
      cam.setOrtho(-m_dist * Globals::aspectRatio - m_viewOffset.x, m_dist * Globals::aspectRatio - m_viewOffset.x, -m_dist + m_viewOffset.y, m_dist + m_viewOffset.y, 0.1f, 100000.0f);
#endif
