...
bool OculusVR::Init() {
  ...
  m_DistortionCaps = ovrDistortionCap_Chromatic | ovrDistortionCap_TimeWarp;
...
void OculusVR::BeginEye(int idx) {
  ...
  m_Projection = ovrMatrix4f_Projection(m_EyeRenderDesc[eye].Fov, 0.1f, 10000.0f, true);
}
...
void OculusVR::DistortionMeshInit() {
  ...
  for (int eyeNum = 0; eyeNum < 2; eyeNum++)
  {
    ...
    for (unsigned vertNum = 0; vertNum < meshData.VertexCount; vertNum++)
    {
      v->Pos.x = ov->Pos.x;
      v->Pos.y = ov->Pos.y;
      v->TexR  = (*(OVR::Vector2f*)&ov->TexR);
      v->TexG  = (*(OVR::Vector2f*)&ov->TexG);
      v->TexB  = (*(OVR::Vector2f*)&ov->TexB);
